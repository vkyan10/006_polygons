...
def draw_polygons( polygons, screen, color ):
    ...
    point = 0
    while point < len(polygons) - 3:
        n = calculate_normal(polygons, point)
        NdotV = dot_product(n, [0,0,1])
...
def add_sphere(polygons, cx, cy, cz, r, steps ):
    ...
    for p0 in range(len(points)):
        ...
        if p2 % n >= 0.01:    # prevent degenerate triangles
            add_polygon(polygons,
                        points[p0][0], points[p0][1], points[p0][2],
                        points[p2][0], points[p2][1], points[p2][2],
...
def add_torus(polygons, cx, cy, cz, r0, r1, steps ):
    ...
    for p0 in range(len(points)):
        p1 = (p0 + 1) % len(points)
        p2 = (p0 + steps + 1) % len(points)
        p3 = (p0 + steps) % len(points)
